...
-- Custom Commands
CMD [[command! SaveAsRoot w !sudo tee % >/dev/null]]
CMD [[command! Realtime set autoread | au CursorHold * checktime | call feedkeys("lh")]]
CMD [[command! ReloadConfig lua require('utils').reload_config()]]
CMD [[command! Reindent lua require('utils').preserve("sil keepj normal! gg=G")]]
CMD [[command! Cls lua require('utils').preserve('%s/\\s\\+$//ge')]]
CMD [[command! BufOnly lua require('utils').preserve("silent! %bd|e#|bd#")]]
...
local autocmds = {
    ...
    attatch_colorizer = { { "BufEnter", "*.css,*.scss,*.js,*.html,*.tsx,*.jsx", [[ColorizerAttachToBuffer]] } },
    mkdir_before_saving = {
